...
- **Messaging is partly owned, and the rows record only the unowned part.** `Reliable-delivery mechanism` is a `spring-boot-patterns` decision and blocks like any other. `Message broker` and `Message ordering guarantee required` have no owner skill, exactly like the caching rows: security, naming, layering, and testing of messaging are owned by their usual skills, but no reference describes the outbox relay, the consumer wiring, the deduplication store, or partitioning. Record the answers when they are made, and do not introduce a broker to fill a row. `_core/README.md` carries the full split of what is owned and what is not — read it before concluding that a messaging rule is missing.
