...
- **Tenancy model** is answered before the first table, because it is the row every other tenant rule reads and the only one on this page that cannot be added later as a setting: moving off `single-tenant` is an expand-and-contract migration of every table, a change to every unique constraint, and a cache key-namespace change. It carries no fallback and therefore blocks, like the database engine and for the same reason: `single-tenant` is a claim about the deployment, not an absence of evidence, and no repository proves it. `application-security` owns what each model changes; `Tenant identifier claim` under **Security** is `n/a` only while this row is `single-tenant`.
- **Soft delete carries the same shape as Tenancy model** and sits beside it deliberately: a guard row whose `none` value is a real answer, and whose other values put a predicate on every query in the project. Both are cheap to record now and expensive to add later, and both are the kind of decision a feature will otherwise make on its own. `spring-data-jpa` owns what each value changes.
- **Timestamp and time-zone policy** is one decision with three faces — the Java type, the column type, and the wire form — and it is recorded once because a project that decides it per entity ends up comparing an `Instant` with a `LocalDateTime` and losing an hour twice a year. `rest-api-contract` documents whichever wire form is chosen.
...
- **Idempotency claim shape** is a `spring-boot-patterns` decision with a security policy above it, so read both rows together: `application-security` decides *when* a key is required and what it is bound to, this row decides how the claim commits. The fallback `single-phase` is correct wherever the effect is a database write, which under this architecture is most of them; `two-phase` is required — not preferred — where the use case makes a synchronous external call whose result the caller receives, and it is the only shape that needs `Idempotency claim lease` filled in — that row is `n/a` under `single-phase`, and a real duration, answered rather than defaulted, under `two-phase`.
...
- **Request budget** is the number the whole **Performance and capacity** table is sized against, and the worked arithmetic under that table is part of the row rather than commentary: changing any bound means redoing those four sums and recording the result. A budget whose parts have never been added up is a number that agrees with nothing.
- **Instance count** has no fallback and blocks, because it is a multiplier rather than a value: every pool in the table is sized as *pool × instances* against a limit somebody else owns — the database's connection limit, the callee's rate limit. A guessed instance count does not size anything, it just makes the multiplication look done.
- **The three outbound rows are one decision in three parts.** `Outbound timeouts` carries connect and read; `Outbound connection acquisition wait` carries the third wait, which is the one projects miss; `Outbound client pool` carries the sizes and has no fallback for the same reason `Connection pool size` has none. `spring-boot-patterns` states why the library defaults cannot be left alone — the two that matter are unset and multi-minute.
- **Ingress request ceiling** has no fallback on purpose. It is the only hard deadline a request has: nothing inside the application ends a synchronous one, so a project that leaves this empty has a **Request budget** that nothing enforces — an agreed number rather than a limit. Record it at or above the budget.
- **Statement cache, JDBC fetch size and JDBC batch size are the cost rows**, not bound rows: they do not stop anything from running away, they decide what a statement costs when it runs normally. They sit in this section anyway because they travel over the same connection channel as the bounds above and are decided in the same change. `Statement cache` defaults to `enabled` because every supported driver ships it off or unhelpfully sized; the two `none` fallbacks mean the driver default applies and somebody looked.
...
- **Dependency version policy** is recorded rather than defaulted because its strict level is the one gate in this set that can fail on somebody else's upgrade. Both build tools reach the same levels; `build-and-dependencies` states which rule belongs to which, and the row exists so that a convergence failure is answered with an explicit resolution instead of by deleting the rule.
- **Security profile and ASVS baseline** has `none` as a legitimate fallback, and that is the point: an absent baseline becomes a visible recorded decision rather than silence that later reads as a compliance claim. Recording `none` says nobody adopted ASVS, not that the project is compliant with it.
