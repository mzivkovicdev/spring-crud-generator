...
- For an optimistic-conflict retry, verify four things the compiler cannot: that the retry advice wraps the transaction advice rather than the reverse, that the backoff has jitter and a maximum delay, that no `catch` below the annotated method swallows the failure the retry exists to observe, and that the retried operation recomputes from re-read state rather than overwriting with values the caller computed earlier. The last is the silent one — the retry succeeds, nothing throws, and a concurrent edit is lost.
- Verify that an exhausted retry produces the project's conflict contract rather than leaking the framework exception as a `500`. The advice must handle the **framework** types — `OptimisticLockingFailureException` and `PessimisticLockingFailureException` — because nothing throws a project exception on either path unless the project chose Spring Retry's `@Recover`. An advice that handles only project types passes review by inspection and returns `500` in production.
- Verify that the two contention conditions are not collapsed into one status: an exhausted optimistic retry is `409`, a lock timeout or deadlock victim is `503` with `Retry-After`. Check the log level too — an expected `5xx` logged at `ERROR` with a stack trace is a log flood waiting for load, not a defect the reader will notice in a quiet environment.
