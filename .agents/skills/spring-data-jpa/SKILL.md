...
- **The two contention failures are different conditions, and neither is caught in a service.** An exhausted optimistic retry means the row genuinely changed under the caller; a lock timeout, deadlock victim, or serialization failure means the row was held and the wait expired. Both surface from the transaction interceptor after the method body returned, so both are translated in the REST exception advice, from the Spring framework types rather than a project exception. `spring-boot-patterns` decides what each one shows the caller — and it gives them **different** statuses, so do not declare them as one condition.
