...
<#assign relationIdType = rel.relationIdType>
    ...
    <#if rel.isCollection>@DeleteMapping("/{id}/${relationName}s/{${relationField}}")<#else>@DeleteMapping("/{id}/${relationName}s")</#if>
    public ResponseEntity<Void> ${rel.methodName}(@PathVariable final ${idType} id<#if rel.isCollection>, @PathVariable final ${relationIdType} ${relationField}</#if>) {
