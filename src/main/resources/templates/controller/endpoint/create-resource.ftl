...
@PostMapping
    public ResponseEntity<${transferObjectClass}> ${uncapModelName}Post(@RequestBody final ${transferObjectClass} body) {
        ...
        <#if rel.isCollection>
        final List<${rel.relationIdType}> ${rel.field}Ids = CollectionUtils.isEmpty(body.${rel.field}) ? 
                List.of() :
                body.${rel.field}()?strema()
                    .map(${relationTransferObject})
        ...
        return ResponseEntity.ok(
            ${mapperClass}.map${modelName?cap_first}To${transferObjectClass}(
                this.${serviceField}.create(<#list inputFields as arg>
                    <#if arg.isRelation>
                        <#if arg.isCollection>
                        ${arg.field}Ids
                        <#else>
