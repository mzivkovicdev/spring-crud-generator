...
<#assign mapperClass = strippedModelName?uncap_first + "Mapper">
    ...
    public ResponseEntity<<#if !swagger>${transferObjectClass}<#else>${strippedModelName}</#if>> ${uncapModelName}sIdPut(<#if !swagger>@PathVariable </#if>final ${idType} id, <#if !swagger>@RequestBody </#if>final <#if !swagger>${transferObjectClass}<#else>${strippedModelName}</#if> body) {
        <#if !swagger>
