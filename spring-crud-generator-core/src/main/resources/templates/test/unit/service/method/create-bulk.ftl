    ...
    void bulkCreate() {
        ...
        when(this.${strippedModelName?uncap_first}Repository.<#if isMongoDB?? && isMongoDB>saveAll<#else>saveAllAndFlush</#if>(${strippedModelName?uncap_first}s))
        ...
        verify(this.${strippedModelName?uncap_first}Repository).<#if isMongoDB?? && isMongoDB>saveAll<#else>saveAllAndFlush</#if>(${strippedModelName?uncap_first}s);
    }
