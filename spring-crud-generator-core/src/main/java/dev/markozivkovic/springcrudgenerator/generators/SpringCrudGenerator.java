...
public class SpringCrudGenerator implements CodeGenerator, ProjectArtifactGenerator {
    ...
    private static final String RATE_LIMITING = "rate-limiting";
    ...
    public SpringCrudGenerator(final CrudConfiguration crudConfiguration, final List<ModelDefinition> entities,
        ...
        this.ARTIFACT_GENERATORS.put(RATE_LIMITING, new RateLimitingGenerator(crudConfiguration, packageConfiguration));
    ...
    /**
     * Registers database-specific generators using the {@link DatabaseSupportRegistry} SPI.
     * Database support implementations are discovered at runtime via {@link java.util.ServiceLoader}.
     *
    ...
    private void registerDatabaseGenerators(final CrudConfiguration crudConfiguration, final List<ModelDefinition> entities,
        ...
        final DatabaseSupport support = DatabaseSupportRegistry.resolve(database);
        final Map<String, CodeGenerator> dbGenerators = support.createCodeGenerators(
            crudConfiguration, entities, projectMetadata, packageConfiguration
        );
        this.GENERATORS.putAll(dbGenerators);
    }
}
