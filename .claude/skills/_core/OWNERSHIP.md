...
| Resource bounds | `spring-data-jpa` owns the database-side bounds: statement and transaction timeouts, pool size, connection wait and lifetime, batch size, page size | `spring-boot-patterns` owns the request budget they all fit inside and the outbound-side bounds — the client's three waits and its connection pool — and `application-security` owns the per-caller limits that stop one client consuming them |
