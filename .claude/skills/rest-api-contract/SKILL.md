...
description: OpenAPI contract ownership and evolution for Java 21+ Spring Boot REST APIs. Use when creating or changing a public endpoint, request or response shape, status code, header, enum value, or error contract; when producing, reviewing, or publishing an OpenAPI document; when deciding whether a change is breaking; and when versioning, deprecating, or retiring an API. Covers code-first and contract-first equally. Excludes GraphQL, gRPC, asynchronous event contracts, and gateway configuration.
---

# REST API Contract

The OpenAPI document is the API's public interface. Once a consumer depends on it, changing it is a
deployment of someone else's software. Treat the contract as the deliverable and the controller as
its implementation, not the other way round.
...
## Choose the authoring direction first

Two directions are supported. They are not interchangeable, and a project uses exactly one per
deployable service.
...
- Read [OpenAPI document](references/openapi-document.md) when producing, structuring, or improving the document itself.
- Read [compatibility and evolution](references/compatibility-and-evolution.md) when changing an existing endpoint, deciding whether a change is breaking, or versioning, deprecating, or retiring an API.
- Read [contract-first generation](references/contract-first-generation.md) only when the project profile records contract-first.

## The contract is part of the change

- A change to a public endpoint is not complete until the contract reflects it. That includes paths, methods, status codes, headers, request and response shapes, enum values, required-ness, and every error condition the endpoint can produce.
...
- Document nullability the way the project's OpenAPI version expresses it, and keep one style throughout. The two OpenAPI versions express it differently; [OpenAPI document](references/openapi-document.md) covers which applies.

## Keep the document honest

A contract that has drifted from the implementation is worse than no contract, because consumers
trust it.
...
Consumers read the document, not the controller. Optional-looking fields are what make it usable:
...
- [ ] The authoring direction is recorded in the project profile and was followed.
- [ ] The contract reflects every path, status, header, field, and error the change introduced or altered.
...
- [ ] The committed document matches the implementation, and the build proves it.
- [ ] Summaries, descriptions, examples, and constraints are present and free of real data.
